...
const {refundDataArray} = data;

// Functional component for the Refund page
const Refund = ({handlePage}) => {
   
    return (
        <div>
            {/* Container for the entire Refund page with specific font family */}
            <Box fontFamily={"Inter"}>
                ...
                {/* Text for displaying the Transactions section title */}
                <Text mt={"44px"} fontSize={"20px"} fontWeight={500}>Transactions | This Month</Text>
                
                {/* Flex container for toggling between Payouts and Refunds */}
                <Flex mt={"20px"} gap={"12px"}>
                    <Text p={"6px 16px"} color={"#808080"} borderRadius={"full"} bgColor={"#E6E6E6"} cursor={"pointer"} onClick={()=>handlePage(true)}>Payouts (22)</Text>
                    <Text p={"6px 16px"} color={"#ffffff"} borderRadius={"full"} bgColor={"#146EB4"} cursor={"pointer"} onClick={()=>handlePage(false)}>Refunds {(refundDataArray.length)}</Text>
                </Flex>
                
                {/* Box for search, sort, and download functionalities */}
                <Box mt={"24px"} px={"12px"} pt={"12px"} pb={"8px"} bgColor={"white"}>
                    <Flex>
                        {/* Input group for search functionality */}
                        <InputGroup>
                            <InputLeftElement pointerEvents="none" children={<RiSearchLine size={"14px"} color="#999999" />} />
                            <Input bgColor={"#FFFFFF"} py={"10px"} px={"16px"} type="text" fontWeight={400} fontSize={"14px"} w={"248px"} boxSizing='border-box' placeholder="Order ID or transaction ID" _placeholder={{ color: "#999999" }} />
                        </InputGroup>
                    ...
                    <Flex color={"#4D4D4D"} fontWeight={500} fontSize={"14px"} mt={"12px"} py={"10px"} px={"12px"} bgColor={"#F2F2F2"} justifyContent={"space-between"} borderRadius={"4px"}>
                    ...
                    {/* Mapping through the refundDataArray to render transaction entries */}
                    {refundDataArray?.map((el,i) => (
                        <Flex key={i} color={"#1A181E"} fontSize={"14px"} py={"14px"} px={"12px"} bgColor={"#ffffff"} justifyContent={"space-between"} borderBottom={(refundDataArray.length-1)==i?"":"1px solid #E6E6E6"} fontWeight={400}>
...
// Exporting the Refund component for use in other parts of the application
